...
const RELATIONSHIP_ARTIFACT_KINDS=new Set(['Association','Aggregation','Composition','Dependency','Realization','Generalization','DeriveReqt','Satisfy','Verify','Refine','Trace','Copy','Allocate','Connector','DelegationConnector','BindingConnector','ItemFlow','ControlFlow','ObjectFlow','InterruptingEdge','Transition','Message','Include','Extend']);
...
export async function importWorkbook(file,project,log,options={}){
  ...
  try{
    ...
    prepareRelationshipEndpointReferences(ordered.filter(s=>s.definition?.role==='relationships'),ctx);
...
export function normalizeImportedTestCaseFields(element){if(element.kind!=='TestCase')return element;element.verificationCaseId=text(element.verificationCaseId)||text(element.externalId)||text(element.id);element.verificationObjective=text(element.verificationObjective)||text(element.documentation)||text(element.name)||'Verify the referenced requirement.';const methods=['Analysis','Demonstration','Inspection','Test'],method=methods.find(value=>normalizedKey(value)===normalizedKey(element.verificationMethod));if(element.verificationMethod&&!method)element.importedVerificationMethod=element.verificationMethod;element.verificationMethod=method||'Test';const levels=['Component','Subsystem','System','Integration','Acceptance'],level=levels.find(value=>normalizedKey(value)===normalizedKey(element.verificationLevel));if(element.verificationLevel&&!level)element.importedVerificationLevel=element.verificationLevel;element.verificationLevel=level||'System';const statuses=['Draft','Planned','Ready','Deferred','Cancelled'],status=statuses.find(value=>normalizedKey(value)===normalizedKey(element.plannedStatus));if(element.plannedStatus&&!status)element.importedPlannedStatus=element.plannedStatus;element.plannedStatus=status||'Draft';element.executionStatus=element.executionStatus||'Unavailable';return element;}
export function coalesceImportedRelationships(ctx){const canonical=new Map(),replacement=new Map(),retained=[];for(const relationship of ctx.project.relationships){const key=[relationship.kind,relationship.sourceId,relationship.targetId].join('\u0000'),existing=canonical.get(key);if(!existing){canonical.set(key,relationship);retained.push(relationship);continue}replacement.set(relationship.id,existing.id);existing.alternateExternalIds=[...new Set([...(existing.alternateExternalIds||[]),relationship.externalId||relationship.id].filter(Boolean))];for(const alias of [relationship.id,relationship.externalId])setAlias(ctx.relationshipAlias,alias,existing.id,true);ctx.report.relationships.skipped++;}if(!replacement.size)return;ctx.project.relationships=retained;for(const diagram of ctx.project.diagrams)for(const edge of diagram.edges||[])if(replacement.has(edge.relationshipId))edge.relationshipId=replacement.get(edge.relationshipId);for(const relationship of retained)for(const field of ['connectorId','realizingRelationshipId'])if(replacement.has(relationship[field]))relationship[field]=replacement.get(relationship[field]);}
...
function resolveElementReferences(ctx){
  for(const element of ctx.pendingElementRefs){
    ...
    if(element.typeRef)element.typeRef=resolveAlias(ctx.elementAlias,element.typeRef)||element.typeRef;
    if(element.parentRequirementId)element.parentRequirementId=resolveAlias(ctx.elementAlias,element.parentRequirementId)||element.parentRequirementId;
    for(const field of ['classifierId','parentInstanceId','configurationId','definingFeatureId','valueInstanceId','rootInstanceId','allocatedBlockId','representedElementId','referencedBehaviorId','coveredLifelineId','startMessageId','finishMessageId'])if(element[field])element[field]=resolveAlias(ctx.elementAlias,element[field])||element[field];
...
function resolveOwner(ctx,raw,qualified){
  const candidate=text(raw)||text(qualified);if(!candidate)return ctx.project.root.id;
  const resolved=resolveAlias(ctx.elementAlias,candidate);if(resolved)return resolved;
  // CATIA workbooks often identify packages by path/name rather than a prior External ID.
  return ensurePackagePath(ctx,candidate);
}
function ensurePackagePath(ctx,path){
  const names=String(path).split(/\s*(?:::|>)\s*/).map(text).filter(Boolean);if(!names.length)return ctx.project.root.id;
  let owner=ctx.project.root.id;
  for(const name of names){
    let pkg=ctx.project.elements.find(e=>e.kind==='Package'&&e.ownerId===owner&&e.name===name);
    if(!pkg){pkg=defaultElement('Package',owner);pkg.id=`pkg-${slug(name)}-${uid('').slice(-8)}`;pkg.externalId=pkg.id;pkg.name=name;pkg.isAutoCreatedByImport=true;ctx.project.elements.push(pkg);ctx.report.elements.created++;addAliases(ctx.elementAlias,pkg,pkg.id);}
    owner=pkg.id;
  }
  return owner;
}

function importConstraintEquations(sheets,ctx){
  for(const sheet of sheets)for(const row of sortedRows(sheet.rows)){
    const ownerId=resolveAlias(ctx.elementAlias,valueFor(row,'equationOwner'));const owner=ctx.project.elements.find(e=>e.id===ownerId);
    if(!owner){ctx.report.warnings.push(`${sheet.name} row ${row.__rowNumber}: constraint block not found.`);continue;}
    owner.constraintExpressions=owner.constraintExpressions||[];
    const id=text(valueFor(row,'externalId'))||uid('equation');const expression=text(valueFor(row,'expression'));
    const existing=owner.constraintExpressions.find(e=>e.id===id);const record={id,expression,sequence:numberOr(valueFor(row,'sequence'),owner.constraintExpressions.length+1),language:text(row['Language/Profile']),status:text(valueFor(row,'status'))};
    if(existing)Object.assign(existing,record);else owner.constraintExpressions.push(record);
  }
}

function importDiagrams(sheets,ctx){
  for(const sheet of sheets)for(const row of sortedRows(sheet.rows)){
    const externalId=text(valueFor(row,'diagramId'))||text(valueFor(row,'externalId'))||uid('diagram');const name=text(valueFor(row,'name'))||externalId;
    let diagram=findByAlias(ctx.project.diagrams,ctx.diagramAlias,externalId);
    if(!diagram){diagram={id:externalId,externalId,name,diagramType:'Block Definition Diagram',ownerId:ctx.project.root.id,contextId:ctx.project.root.id,nodes:[],edges:[],documentation:''};ctx.project.diagrams.push(diagram);ctx.report.diagrams.created++;}else ctx.report.diagrams.updated++;
    const ownerRaw=text(valueFor(row,'owner')),contextRaw=text(valueFor(row,'context'));
    Object.assign(diagram,{name,diagramType:normalizeDiagramType(valueFor(row,'diagramType')),ownerId:resolveOwner(ctx,ownerRaw,valueFor(row,'ownerQualifiedName')),contextId:resolveAlias(ctx.elementAlias,contextRaw)||resolveOwner(ctx,contextRaw||ownerRaw,''),documentation:text(valueFor(row,'documentation'))||text(row['CATIA Build Note']),canvasWidth:numberOr(valueFor(row,'canvasWidth'),diagram.canvasWidth||2400),canvasHeight:numberOr(valueFor(row,'canvasHeight'),diagram.canvasHeight||1600),autoLayout:booleanValue(valueFor(row,'autoLayout')),importSource:{file:ctx.fileName,sheet:sheet.name,row:row.__rowNumber}});
    addAliases(ctx.diagramAlias,diagram,diagram.id);
  }
}

function importRelationships(sheets,ctx){
  for(const sheet of [...sheets].sort((a,b)=>relationshipSheetPriority(a)-relationshipSheetPriority(b)))for(const row of sortedRows(sheet.rows)){
    const kind=normalizeRelationshipKind(valueFor(row,'relationshipKind'),sheet.definition.relationshipKind||'Dependency');
    if(normalizedKey(sheet.name).includes('sysmlrelationships')&&(['Connector','ItemFlow'].includes(kind)||['connector','itemflow'].includes(normalizedKey(row['Source Metaclass'])))){ctx.report.relationships.skipped++;continue;}
    if(kind==='ItemFlow'){
    ...
    const externalId=text(valueFor(row,'relationshipId'))||text(valueFor(row,'externalId'))||uid('rel');
    let rel=findByAlias(ctx.project.relationships,ctx.relationshipAlias,externalId);
    if(!rel){rel=defaultRelationship(kind,sourceId,targetId,ctx.project.root.id);rel.id=externalId;ctx.project.relationships.push(rel);ctx.report.relationships.created++;}else ctx.report.relationships.updated++;
    Object.assign(rel,{externalId,name:text(valueFor(row,'name')),kind,sourceId,targetId,ownerId:resolveOwner(ctx,valueFor(row,'owner'),valueFor(row,'ownerQualifiedName')),documentation:text(valueFor(row,'documentation')),sourceRole:text(valueFor(row,'sourceRole')),targetRole:text(valueFor(row,'targetRole')),sourceMultiplicity:text(valueFor(row,'sourceMultiplicity'))||'1',targetMultiplicity:text(valueFor(row,'targetMultiplicity'))||'1',sourceNavigable:booleanValue(valueFor(row,'sourceNavigable')),targetNavigable:booleanValue(valueFor(row,'targetNavigable')),connectorTypeRef:resolveAlias(ctx.elementAlias,valueFor(row,'connectorType'))||text(valueFor(row,'connectorType')),connectorKind:text(row['Connector Kind']||row['Kind'])||((kind==='DelegationConnector')?'delegation':'assembly'),sourcePortId:resolveAlias(ctx.elementAlias,valueFor(row,'sourcePort'))||text(valueFor(row,'sourcePort')),targetPortId:resolveAlias(ctx.elementAlias,valueFor(row,'targetPort'))||text(valueFor(row,'targetPort')),sourcePartWithPortPath:splitIds(valueFor(row,'sourcePartWithPort')).map(id=>resolveAlias(ctx.elementAlias,id)||id),targetPartWithPortPath:splitIds(valueFor(row,'targetPartWithPort')).map(id=>resolveAlias(ctx.elementAlias,id)||id),conveyedIds:splitIds(valueFor(row,'conveyedIds')).map(id=>resolveAlias(ctx.elementAlias,id)||id),guard:text(valueFor(row,'guard')),effect:text(valueFor(row,'effect')),trigger:text(valueFor(row,'trigger')),triggerIds:splitIds(valueFor(row,'trigger')).map(id=>resolveAlias(ctx.elementAlias,id)||id),messageSort:text(valueFor(row,'messageSort')),sequenceOrder:numberOr(valueFor(row,'sequence'),rel.sequenceOrder??0),operationRef:resolveAlias(ctx.elementAlias,text(row['Operation ID']))||text(row['Operation ID']),signalRef:resolveAlias(ctx.elementAlias,text(row['Signal ID']))||text(row['Signal ID']),carriedTypeId:resolveAlias(ctx.elementAlias,text(row['Carried Type ID']||row['Object Type ID']))||text(row['Carried Type ID']||row['Object Type ID']),extensionPointId:text(row['Extension Point ID']),importSource:{file:ctx.fileName,sheet:sheet.name,row:row.__rowNumber}});
    rel.sourceEndpointPath=[...rel.sourcePartWithPortPath,...(rel.sourcePortId?[rel.sourcePortId]:[rel.sourceId])];rel.targetEndpointPath=[...rel.targetPartWithPortPath,...(rel.targetPortId?[rel.targetPortId]:[rel.targetId])];
    addAliases(ctx.relationshipAlias,rel,rel.id);
    const diagramId=resolveAlias(ctx.diagramAlias,valueFor(row,'diagramId'))||text(valueFor(row,'diagramId'));const diagram=ctx.project.diagrams.find(d=>d.id===diagramId);if(diagram)addRelationshipPresentation(diagram,rel,ctx);
  }
}

function prepareRelationshipEndpointReferences(sheets,ctx){
  for(const sheet of sheets.filter(item=>item.definition.allowEndpointReferences))for(const row of sortedRows(sheet.rows)){
    const endpoints=relationshipEndpoints(sheet,row,ctx);
    ensureRelationshipEndpointReference(ctx,endpoints.source,text(row['Source Unit']||row['Source Name']),sheet,row);
    ensureRelationshipEndpointReference(ctx,endpoints.target,text(row['Target Unit']||row['Target Name']),sheet,row);
  }
}

function ensureRelationshipEndpointReference(ctx,externalId,name,sheet,row){
  if(!externalId||resolveAlias(ctx.elementAlias,externalId))return;
  const element=defaultElement('Block',ctx.project.root.id);
  Object.assign(element,{id:externalId,externalId,name:name||externalId,kind:'Block',isExternalReference:true,importSource:{file:ctx.fileName,sheet:sheet.name,row:row.__rowNumber,profile:'catia-cameo-workbook-v2-fsbs-grindavik'}});
  ctx.project.elements.push(element);ctx.report.elements.created++;addAliases(ctx.elementAlias,element,externalId);
}

function relationshipSheetPriority(sheet){const name=normalizedKey(sheet.name);if(name.includes('connector')&&!name.includes('itemflow'))return 0;if(name.includes('itemflow'))return 1;if(name.includes('sysmlrelationships'))return 3;return 2;}
function relationshipEndpoints(sheet,row,ctx){
  const n=normalizedKey(sheet.name);
  if(n.includes('generalization'))return{source:text(row['Specific External ID']||valueFor(row,'source')),target:text(row['General External ID']||valueFor(row,'target'))};
  if(n.includes('transition')){
    const machineId=text(row['State Machine ID']),source=text(row['Source State ID']||valueFor(row,'source')),target=text(row['Target State ID']||valueFor(row,'target'));
    return{source:normalizedKey(source)==='initial'?ensureInitialPseudostate(ctx,machineId,sheet,row):source,target:normalizedKey(target)==='initial'?ensureInitialPseudostate(ctx,machineId,sheet,row):target};
  }
  if(n.includes('connector')&&text(row['Source Port ID']))return{source:text(row['Source Port ID']),target:text(row['Target Port ID'])};
  if(n.includes('itemflow'))return{source:text(row['Source Port ID']||row['Connector ID']||valueFor(row,'source')),target:text(row['Target Port ID']||row['Connector ID']||valueFor(row,'target'))};
  if(n.includes('bindingconnector'))return{source:text(row['Constraint Parameter ID']||row['Constraint Property ID']),target:text(row['Bound Value Property ID'])};
  if(n.includes('configurationmembership'))return{source:text(row['Configuration ID']),target:text(row['Instance ID'])};
  return{source:text(valueFor(row,'source')),target:text(valueFor(row,'target'))};
}

function ensureInitialPseudostate(ctx,machineId,sheet,row){
  const externalId=`${machineId||'state-machine'}.INITIAL`,existing=resolveAlias(ctx.elementAlias,externalId);if(existing)return existing;
  const element=defaultElement('InitialPseudostate',resolveAlias(ctx.elementAlias,machineId)||ctx.project.root.id);
  Object.assign(element,{id:externalId,externalId,name:'Initial',kind:'InitialPseudostate',importSource:{file:ctx.fileName,sheet:sheet.name,row:row.__rowNumber,profile:'catia-cameo-workbook-v2-fsbs-grindavik'}});
  ctx.project.elements.push(element);ctx.report.elements.created++;addAliases(ctx.elementAlias,element,externalId);return externalId;
}

function createDerivedRelationships(ctx){
  for(const item of ctx.pendingDerived){
    const sourceId=resolveAlias(ctx.elementAlias,item.source),targetId=resolveAlias(ctx.elementAlias,item.target);if(!sourceId||!targetId)continue;
    const id=`derived-${slug(item.kind)}-${slug(item.source)}-${slug(item.target)}`;if(ctx.project.relationships.some(r=>r.id===id))continue;
    const rel=defaultRelationship(item.kind,sourceId,targetId,resolveOwner(ctx,item.owner,''));rel.id=id;rel.externalId=id;rel.isDerivedFromImport=true;ctx.project.relationships.push(rel);addAliases(ctx.relationshipAlias,rel,id);ctx.report.relationships.created++;
  }
}

function importConnectorEnds(sheets,ctx){for(const sheet of sheets)for(const row of sortedRows(sheet.rows)){
  const relationshipId=resolveAlias(ctx.relationshipAlias,valueFor(row,'relationshipId')||valueFor(row,'externalId'));const rel=ctx.project.relationships.find(r=>r.id===relationshipId);if(!rel){ctx.report.warnings.push(`${sheet.name} row ${row.__rowNumber}: connector not found.`);continue;}
  const end=text(row['End']||row['Connector End']||'').toLowerCase();const property=text(row['Part With Port ID']||row['Property Path']||row['Part Property ID']);const port=text(row['Port ID']);const path=splitIds(property).map(id=>resolveAlias(ctx.elementAlias,id)||id);
  if(end.includes('target')||end==='2'){rel.targetPartWithPortPath=path;if(port)rel.targetPortId=resolveAlias(ctx.elementAlias,port)||port;}else{rel.sourcePartWithPortPath=path;if(port)rel.sourcePortId=resolveAlias(ctx.elementAlias,port)||port;}
}}

function importDiagramNodes(sheets,ctx){for(const sheet of sheets)for(const row of sortedRows(sheet.rows)){
  const diagramId=resolveAlias(ctx.diagramAlias,valueFor(row,'diagramId'))||text(valueFor(row,'diagramId'));
  const rawElement=valueFor(row,'semanticElement')||valueFor(row,'externalId')||valueFor(row,'sourceElement');const elementId=resolveAlias(ctx.elementAlias,rawElement);
  const diagram=ctx.project.diagrams.find(d=>d.id===diagramId);if(!diagram||!elementId){ctx.report.warnings.push(`${sheet.name} row ${row.__rowNumber}: unresolved diagram presentation.`);continue;}
...
function importDiagramEdges(sheets,ctx){for(const sheet of sheets)for(const row of sortedRows(sheet.rows)){
  const diagramId=resolveAlias(ctx.diagramAlias,valueFor(row,'diagramId'))||text(valueFor(row,'diagramId'));const relationshipId=resolveAlias(ctx.relationshipAlias,valueFor(row,'semanticEdge')||valueFor(row,'relationshipId')||valueFor(row,'externalId'));
  const diagram=ctx.project.diagrams.find(d=>d.id===diagramId),relationship=ctx.project.relationships.find(r=>r.id===relationshipId);if(!diagram||!relationship){ctx.report.warnings.push(`${sheet.name} row ${row.__rowNumber}: unresolved diagram edge.`);continue;}
  addRelationshipPresentation(diagram,relationship,ctx,{id:text(valueFor(row,'edgePresentationId'))||uid('edge'),points:parseBendPoints(valueFor(row,'bendPoints')),routingStyle:text(valueFor(row,'routingStyle'))||'orthogonal',label:text(valueFor(row,'labelText')),occurrenceY:numberOr(row['Occurrence Y']||row['Message Y'],undefined),labelPosition:(row['Label X']!=null||row['Label Y']!=null)?{x:numberOr(row['Label X'],0),y:numberOr(row['Label Y'],0)}:undefined});
}}

function expandDiagramLists(sheets,ctx){for(const sheet of sheets)for(const row of sheet.rows){
  const diagramId=resolveAlias(ctx.diagramAlias,valueFor(row,'diagramId')||valueFor(row,'externalId'));const diagram=ctx.project.diagrams.find(d=>d.id===diagramId);if(!diagram)continue;
  for(const rawId of splitIds(valueFor(row,'displayedElementIds'))){const elementId=resolveAlias(ctx.elementAlias,rawId);if(elementId&&!diagram.nodes.some(n=>n.elementId===elementId))autoNode(diagram,elementId,ctx);}
  for(const rawId of splitIds(valueFor(row,'relationshipIds'))){const relId=resolveAlias(ctx.relationshipAlias,rawId);const rel=ctx.project.relationships.find(r=>r.id===relId);if(rel)addRelationshipPresentation(diagram,rel,ctx);}
}}

function importNavigation(sheets,ctx){for(const sheet of sheets)for(const row of sortedRows(sheet.rows)){
  const sourceId=resolveAlias(ctx.elementAlias,valueFor(row,'sourceElement'));const targetId=resolveAlias(ctx.diagramAlias,valueFor(row,'targetDiagram')||valueFor(row,'childDiagram'));const parentId=resolveAlias(ctx.diagramAlias,valueFor(row,'parentDiagram'));
  const source=ctx.project.elements.find(e=>e.id===sourceId),target=ctx.project.diagrams.find(d=>d.id===targetId),parent=ctx.project.diagrams.find(d=>d.id===parentId);
  if(source&&target){source.childDiagramIds=[...new Set([...(source.childDiagramIds||[]),target.id])];source.primaryChildDiagramId=booleanValue(valueFor(row,'isPrimary'))?target.id:(source.primaryChildDiagramId||target.id);source.navigationLabel=text(valueFor(row,'navigationLabel'));ctx.report.navigation.created++;}
  if(parent&&target){parent.childDiagramIds=[...new Set([...(parent.childDiagramIds||[]),target.id])];ctx.report.navigation.created++;}
}}

function validateImportGraph(ctx){
  const elementIds=new Set([ctx.project.root.id,...ctx.project.elements.map(e=>e.id)]),relationshipIds=new Set(ctx.project.relationships.map(r=>r.id));
  for(const rel of ctx.project.relationships){const openMessageEnd=rel.kind==='Message'&&(rel.messageSort==='lost'&&!rel.targetId||rel.messageSort==='found'&&!rel.sourceId);if((!elementIds.has(rel.sourceId)||!elementIds.has(rel.targetId))&&!openMessageEnd)ctx.report.errors.push(`Relationship ${rel.id} has unresolved endpoints.`);}
  for(const diagram of ctx.project.diagrams){if(!elementIds.has(diagram.ownerId))ctx.report.errors.push(`Diagram ${diagram.name} has unresolved owner ${diagram.ownerId}.`);if(!elementIds.has(diagram.contextId))ctx.report.warnings.push(`Diagram ${diagram.name} has unresolved context ${diagram.contextId}.`);for(const edge of diagram.edges||[])if(!relationshipIds.has(edge.relationshipId))ctx.report.errors.push(`Diagram ${diagram.name} references missing relationship ${edge.relationshipId}.`);}
  for(const req of ctx.project.elements.filter(e=>e.kind==='Requirement'))if(!text(req.requirementText))ctx.report.warnings.push(`Requirement ${req.externalId||req.id} has blank text.`);
}
