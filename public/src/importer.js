...
function attachBehaviorFields(element,row){
  ...
  const allocated=text(valueFor(row,'allocatedBlock'));if(allocated)element.allocatedBlockId=allocated;
...
function resolveElementReferences(ctx){
  for(const element of ctx.pendingElementRefs){
    ...
    for(const field of ['classifierId','parentInstanceId','configurationId','definingFeatureId','valueInstanceId','rootInstanceId','allocatedBlockId'])if(element[field])element[field]=resolveAlias(ctx.elementAlias,element[field])||element[field];
...
function importRelationships(sheets,ctx){
  for(const sheet of sheets)for(const row of sortedRows(sheet.rows)){
    ...
    if(kind==='ItemFlow'){
      ...
      if(connector){const externalId=text(valueFor(row,'relationshipId'))||text(valueFor(row,'externalId'))||uid('flow');let flow=findByAlias(ctx.project.relationships,ctx.relationshipAlias,externalId);if(!flow){flow=defaultRelationship('ItemFlow',connector.sourceId,connector.targetId,connector.ownerId);flow.id=externalId;ctx.project.relationships.push(flow);ctx.report.relationships.created++}else ctx.report.relationships.updated++;Object.assign(flow,{externalId,connectorId:connector.id,sourceId:connector.sourceId,targetId:connector.targetId,conveyedIds:splitIds(valueFor(row,'conveyedIds')||row['Conveyed Classifier ID']||row['Item ID']).map(id=>resolveAlias(ctx.elementAlias,id)||id),direction:text(row['Direction']||row['Item Flow Direction'])||'sourceToTarget',importSource:{file:ctx.fileName,sheet:sheet.name,row:row.__rowNumber}});connector.itemFlowIds=connector.itemFlowIds||[];if(!connector.itemFlowIds.includes(flow.id))connector.itemFlowIds.push(flow.id);addAliases(ctx.relationshipAlias,flow,flow.id);continue}
    }
    const endpoints=relationshipEndpoints(sheet,row,ctx);if(!endpoints.source||!endpoints.target){ctx.report.warnings.push(`${sheet.name} row ${row.__rowNumber}: blank relationship endpoint.`);continue;}
    ...
    if(!ctx.project.elements.some(e=>e.id===sourceId)&&ctx.project.root.id!==sourceId){ctx.report.errors.push(`${sheet.name} row ${row.__rowNumber}: unresolved source '${endpoints.source}'.`);continue;}
    if(!ctx.project.elements.some(e=>e.id===targetId)&&ctx.project.root.id!==targetId){ctx.report.errors.push(`${sheet.name} row ${row.__rowNumber}: unresolved target '${endpoints.target}'.`);continue;}
...
function importDiagramNodes(sheets,ctx){for(const sheet of sheets)for(const row of sortedRows(sheet.rows)){
  ...
  Object.assign(node,{x:numberOr(valueFor(row,'x'),80),y:numberOr(valueFor(row,'y'),80),width:numberOr(valueFor(row,'width'),190),height:numberOr(valueFor(row,'height'),110),zIndex:numberOr(row['Z Order'],0),parentPresentationId,boundaryOwnerNodeId:parentPresentationId||node.boundaryOwnerNodeId,portSide:text(row['Port Side']||row['Boundary Side']).toLowerCase()||node.portSide,perimeterOffset:numberOr(row['Perimeter Offset']??row['Relative Position'],node.perimeterOffset),endpointPath:splitIds(row['Endpoint Path']||row['Nested Property Path']).map(id=>resolveAlias(ctx.elementAlias,id)||id),compartmentDisplay:text(row['Compartment Display']),stereotypeDisplay:text(row['Stereotype Display']),layer:text(row['Layer']||'default')});ctx.presentationAlias.set(presentationId,node.id);
}}
...
function importDiagramEdges(sheets,ctx){for(const sheet of sheets)for(const row of sortedRows(sheet.rows)){
  ...
  addRelationshipPresentation(diagram,relationship,ctx,{id:text(valueFor(row,'edgePresentationId'))||uid('edge'),points:parseBendPoints(valueFor(row,'bendPoints')),routingStyle:text(valueFor(row,'routingStyle'))||'orthogonal',label:text(valueFor(row,'labelText'))});
}}
...
function validateImportGraph(ctx){
  ...
  for(const rel of ctx.project.relationships){if(!elementIds.has(rel.sourceId)||!elementIds.has(rel.targetId))ctx.report.errors.push(`Relationship ${rel.id} has unresolved endpoints.`);}
...
function addRelationshipPresentation(diagram,relationship,ctx,overrides={}){let edge=diagram.edges.find(e=>e.relationshipId===relationship.id);if(!edge){ensureNode(diagram,relationship.sourceId,ctx);ensureNode(diagram,relationship.targetId,ctx);edge={id:overrides.id||uid('edge'),relationshipId:relationship.id,sourceId:relationship.sourceId,targetId:relationship.targetId,points:[]};diagram.edges.push(edge);ctx.report.presentations.created++;}Object.assign(edge,overrides);}
function ensureNode(diagram,elementId,ctx){if(diagram.nodes.some(n=>n.elementId===elementId))return;autoNode(diagram,elementId,ctx);}
function autoNode(diagram,elementId,ctx){const i=diagram.nodes.length;diagram.nodes.push({id:uid('node'),elementId,x:80+(i%5)*220,y:80+Math.floor(i/5)*150,width:190,height:110});ctx.report.presentations.created++;}
