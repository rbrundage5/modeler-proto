...
export function applyOperation(project,operation){
  switch(operation.type){
    ...
    case'set-relationship-label':{const edge=required(findDiagram(project,operation.diagramId)?.edges.find(e=>e.id===operation.edgeId),'Edge not found');setRelationshipLabel(edge,operation.position);edge.labelPosition=deepClone(operation.position);edge.labelAnchor=deepClone(operation.anchor??null);break}
