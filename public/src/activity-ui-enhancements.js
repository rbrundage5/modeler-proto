...
document.addEventListener('pointerup',event=>{if(reconnectSnapshot)setTimeout(verifyReconnect,0)},true);
document.addEventListener('keydown',event=>{if(event.target.closest?.('#canvas .edge-endpoint'))rememberReconnect()},true);
document.addEventListener('keyup',event=>{if(reconnectSnapshot)setTimeout(verifyReconnect,0)},true);
...
function decorateActivityNotation(){
  const service=api(),canvas=document.getElementById('canvas');if(!service||!canvas)return;canvas.querySelectorAll('.interrupting-edge-glyph').forEach(node=>node.remove());
