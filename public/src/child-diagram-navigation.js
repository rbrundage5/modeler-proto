...
const preferredChildType={Block:'Internal Block Diagram',Activity:'Activity Diagram',StateMachine:'State Machine Diagram',Interaction:'Sequence Diagram'};
...
function navigateToDiagram(diagramId){
  ...
  next.activeDiagramId=diagramId;service.setProject(next);
  const select=$('diagramSelect');if(select)select.value=diagramId;
  globalThis.dispatchEvent(new CustomEvent('systems-modeler-diagram-navigated',{detail:{from:previous,to:diagramId}}));return true;
}

function diagramsPresentingElement(p,elementId){
  return (p?.diagrams||[]).filter(diagram=>(diagram.nodes||[]).some(node=>node.elementId===elementId));
}

function revealElementFromTree(elementId){
  const p=project();if(!p||!elementId)return false;
  const candidates=diagramsPresentingElement(p,elementId);if(!candidates.length){api()?.log?.('This element has no diagram presentation to reveal.','warn');return false}
  const current=candidates.find(diagram=>diagram.id===p.activeDiagramId),target=current||candidates.find(diagram=>diagram.isPrimary===true)||candidates.slice().sort((a,b)=>String(a.name||a.id).localeCompare(String(b.name||b.id)))[0];
  navigateToDiagram(target.id);
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    const group=$('canvas')?.querySelector(`g.node[data-semantic-id="${CSS.escape(elementId)}"]`);if(!group)return;
    group.classList.add('tree-reveal-target');
    try{group.scrollIntoView({block:'center',inline:'center',behavior:'smooth'})}catch{group.scrollIntoView?.()}
    setTimeout(()=>group.classList.remove('tree-reveal-target'),1800);
  }));
  return true;
}
...
function glyphFor(nodeGroup,node,diagram,count){
  ...
  const g=svgElement('g',{class:'child-diagram-glyph',transform:`translate(${x} ${y})`,role:'button',tabindex:'0','aria-label':`Open child diagram ${diagram.name||diagram.id}`,'data-child-diagram-id':diagram.id,'data-child-diagram-count':count});g.style.cursor='pointer';
...
function handleTreeDoubleClick(event){
  ...
  event.preventDefault();event.stopImmediatePropagation();event.stopPropagation();revealElementFromTree(elementId);
}
...
function bind(){const canvas=$('canvas'),tree=$('tree');if(!canvas||!tree)return false;canvas.removeEventListener('dblclick',handleCanvasDoubleClick,true);canvas.addEventListener('dblclick',handleCanvasDoubleClick,true);tree.removeEventListener('dblclick',handleTreeDoubleClick,true);tree.addEventListener('dblclick',handleTreeDoubleClick,true);observer?.disconnect();observer=new MutationObserver(queueRender);observeCanvas();const select=$('diagramSelect');if(select){select.removeEventListener('change',queueRender);select.addEventListener('change',queueRender)}queueRender();return true}
...
globalThis.SystemsModelerChildDiagrams={childDiagramsFor,preferredChildDiagram,render:renderChildDiagramGlyphs,navigate:navigateToDiagram,revealElement:revealElementFromTree};
