...
function glyphFor(nodeGroup,node,diagram,count){
  const size=22,pad=5,x=Math.max(3,(Number(node.width)||120)-size-pad),y=Math.max(3,(Number(node.height)||70)-size-pad),g=svgElement('g',{class:'child-diagram-glyph',transform:`translate(${x} ${y})`,role:'button',tabindex:'0','aria-label':`Open child diagram ${diagram.name||diagram.id}`,'data-child-diagram-id':diagram.id,'data-child-diagram-count':count});g.style.cursor='pointer';
  g.append(svgElement('rect',{x:0,y:0,width:size,height:size,rx:2,fill:'#fff',stroke:'#263746','stroke-width':1.4}));g.append(svgElement('path',{d:'M11 4 V9 M5 9 H17 M5 9 V16 M17 9 V16 M11 9 V16',fill:'none',stroke:'#263746','stroke-width':1.5,'stroke-linecap':'round','stroke-linejoin':'round'}));for(const cx of [5,11,17])g.append(svgElement('rect',{x:cx-2,y:15,width:4,height:4,fill:'#fff',stroke:'#263746','stroke-width':1}));if(count>1){const badge=svgElement('text',{x:18,y:6,'text-anchor':'middle','font-size':'7','font-weight':'700',fill:'#263746'});badge.textContent=String(count);g.append(badge)}
  const open=event=>{event.preventDefault();event.stopImmediatePropagation?.();event.stopPropagation();navigateToDiagram(diagram.id)};g.addEventListener('click',open);g.addEventListener('dblclick',open);g.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' ')open(event)});nodeGroup.append(g)
}
...
function handleTreeDoubleClick(event){const row=event.target?.closest?.('.tree-row[data-type="element"]');if(!row||event.target?.closest?.('.tree-disclosure'))return;const elementId=row.dataset.id;if(!elementId)return;event.preventDefault();event.stopImmediatePropagation();event.stopPropagation();revealElementFromTree(elementId)}
...
function bind(){const canvas=$('canvas'),tree=$('tree');if(!canvas||!tree)return false;canvas.removeEventListener('dblclick',handleCanvasDoubleClick,true);canvas.addEventListener('dblclick',handleCanvasDoubleClick,true);tree.removeEventListener('dblclick',handleTreeDoubleClick,true);tree.addEventListener('dblclick',handleTreeDoubleClick,true);tree.removeEventListener('click',preserveDiagramTabSelection);tree.addEventListener('click',preserveDiagramTabSelection);observer?.disconnect();observer=new MutationObserver(queueRender);observeCanvas();const select=$('diagramSelect');if(select){select.removeEventListener('change',queueRender);select.addEventListener('change',queueRender)}queueRender();return true}
