...
test('every diagram palette entry has a dedicated reusable SVG notation icon',()=>{const audit=auditSysmlNotation();assert.equal(audit.passed,true,JSON.stringify(audit.issues));assert.equal(audit.supportedElements,71);assert.equal(audit.supportedRelationships,33);for(const diagram of Object.values(DIAGRAMS))for(const kind of [...diagram.elements,...diagram.relationships]){assert.equal(hasSysmlIcon(kind),true,kind);assert.match(sysmlPaletteIcon(kind),/^<svg/);assert.doesNotMatch(sysmlPaletteIcon(kind),/>\?</)}});
