...
test('boundary elements stay attached during pointer, keyboard, and property-panel movement',async()=>{const source=await readFile(new URL('../public/src/app.js',import.meta.url),'utf8');assert.match(source,/if\(item\.node\.boundary\)snapBoundary/);assert.match(source,/nodes\.forEach\(node=>.*if\(node\.boundary\)snapBoundary/s);assert.match(source,/node\.boundary&&\['x','y'\]\.includes\(key\)\)snapBoundary/);assert.match(source,/boundaryOwnerNodeId\|\|node\.id===n\.parentPresentationId/)});

test('placement and relationship errors enumerate compatible graphical targets',async()=>{const source=await readFile(new URL('../public/src/app.js',import.meta.url),'utf8');assert.match(source,/Compatible source:/);assert.match(source,/Compatible target:/);assert.match(source,/Compatible endpoints: Part Property/)});

test('direct element tools create at clicks over existing nodes instead of silently selecting them',async()=>{const source=await readFile(new URL('../public/src/app.js',import.meta.url),'utf8');assert.match(source,/if\(placementMode==='DIRECT'\)\{createAt\(\$\('canvas'\),d,ev\);return\}/)});
